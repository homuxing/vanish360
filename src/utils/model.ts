...
import { ref } from 'vue'
import { getItem, setItem, hasItem } from './db'
...
export async function isModelCached(): Promise<boolean> {
  try {
    return await hasItem(MODEL_KEY)
  } catch {
    return false
  }
}
...
export async function loadModel(modelUrl: string = DEFAULT_MODEL_URL): Promise<void> {
  ...
  try {
    ...
    let modelBuffer = await getItem<ArrayBuffer>(MODEL_KEY)

    if (modelBuffer) {
      console.log(`[loadModel] 从缓存加载, 大小: ${(modelBuffer.byteLength / 1024 / 1024).toFixed(2)}MB`)
      modelProgress.value = 100
    } else {
      console.log('[loadModel] 缓存未命中, 开始下载')
      modelBuffer = await downloadModel(modelUrl)
      try {
        await setItem(MODEL_KEY, modelBuffer)
        console.log('[loadModel] 模型已缓存')
      } catch {
        // 缓存失败不影响正常流程
      }
    }
